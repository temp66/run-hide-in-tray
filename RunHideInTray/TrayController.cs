...
internal class TrayController : IDisposable
{
    ...
    void OnExitMenuItemClick(object? sender, EventArgs eventArgs)
    {
        ...
        IEnumerable<Process> processList = _processDescendantsMonitor.GetProcessList();
        using var processEnumerator = processList.GetEnumerator();
        while (true)
        {
            bool result;
            try
            {
                result = processEnumerator.MoveNext();
            }
            ...
            if (!result)
                break;
            using Process process = processEnumerator.Current;
